        ...
        private async Task OnMessage(DocumentMessageDto msg, ulong deliveryTag, CancellationToken ct)
        {
            using (logger.BeginScope(new Dictionary<string, object> { ["CorrelationId"] = msg.CorrelationId }))
            {
                logger.LogInformation(
                    "Processing GenAI summary and tags for document {id} (CorrelationId: {CorrelationId})",
                    msg.DocumentId, msg.CorrelationId);
                ...
                {
                    // Check if summary exists before generating to save tokens.
                    // Handle 404 errors explicitly to avoid poison message loops.
                    ...
                    catch (ServiceException ex)
                    {
                        ...
                        {
                            logger.LogError(
                                "Permanent error encountered (404/NotFound). Skipping document {id} to avoid poison message loop.",
                                msg.DocumentId);
                ...
                {
                    logger.LogInformation("Document {id} already has a summary, skipping summary generation",
                        msg.DocumentId);
                }
                ...
                catch (ServiceException ex)
                {
                    ...
                    {
                        logger.LogWarning(
                            "Tag generation failed with 404 (Model not found). Skipping tags for document {id}.",
                            msg.DocumentId);
                    }
                    else
                    {
