...
namespace BL.Services
{
    public class AccessLogService(IAccessLogRepository accessLogRepo, IMapper mapper, ILogger<AccessLogService> logger)
    {
        ...
        {
            logger.LogInformation("AccessLogService.GetAllAsync called");
            try
            {
        ...
        {
            logger.LogInformation("AccessLogService.GetByIdAsync called with ID={Id}", id);
            try
            {
        ...
        {
            logger.LogInformation("AccessLogService.GetByDocumentIdAsync called for Document ID={Id}", documentId);
            try
            {
        ...
        {
            logger.LogInformation("AccessLogService.AddAsync called for Document ID={Id}", accessLog.Id);
            try
            {
        ...
        {
            logger.LogInformation("AccessLogService.UpdateAsync called for AccessLog ID={Id}", accessLog.Id);
            try
            {
        ...
        {
            logger.LogInformation("AccessLogService.DeleteAsync called for ID={Id}", id);
            try
            {
                AccessLog? existing = await _accessLogRepo.GetByIdAsync(id);
                if (existing == null)
                {
                    logger.LogWarning("AccessLogService.DeleteAsync: AccessLog ID={Id} not found", id);
                    return false;
                }
        ...
        {
            logger.LogInformation("AccessLogService.LogAccessAsync called for Document ID={Id} Date={Date}", documentId,
